...
import { useRoqPlatformUserProfiles, useEquipmentCount, useRentalCount, usePaymentCount } from 'lib/roq';

import { EquipmentListPage as ShowCaseList } from 'pages/equipment';

function DashboardPage() {
  const { session } = useSession();

  const equipmentStat = useEquipmentCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Equipment',
      result: equipmentStat?.data,
      isLoading: equipmentStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`equipment`} passHref legacyBehavior>
                  <Button
