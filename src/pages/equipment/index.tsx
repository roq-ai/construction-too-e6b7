...
import { useRoqClient, useEquipmentFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<EquipmentInterface, unknown>;
...
export function EquipmentListPage(props: EquipmentListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['company', 'user', 'rental.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['name.contains', 'type.contains', 'status.contains', 'location.contains'],
          ...params.filters,
        },
        'equipment',
      ),
  ...
  const { data, error, isLoading, mutate } = useEquipmentFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.equipment.delete({ where: { id } });
      await mutate();
    } catch (error) {
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/equipment/view/${record.id}`} passHref legacyBehavior>
                <Button
