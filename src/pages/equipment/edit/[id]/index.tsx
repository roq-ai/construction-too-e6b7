...
import { useRoqClient, useEquipmentFindFirst } from 'lib/roq';
...
function EquipmentEditPage() {
  ...
  const { data, error, isLoading, mutate } = useEquipmentFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: EquipmentInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.equipment.update({
        data: values as RoqTypes.equipment,
        where: {
          id,
        },
      });
